package au.csiro.data61.gnaf.search
...
object Search {
  ...
  case class CliOption(indexDir: File, bulk: Int, numHits: Int, fuzzyMinLength: Int, fuzzyMaxEdits: Int, fuzzyPrefixLength: Int, interface: String, port: Int)
  val defaultCliOption = {
    val c = ConfigFactory.load.getConfig("gnafSearch")
    def gs(n: String) = c.getString(n)
    def gi(n: String) = c.getInt(n)
    CliOption(new File(gs("indexDir")), gi("bulk"), gi("numHits"), gi("fuzzyMinLength"), gi("fuzzyMaxEdits"), gi("fuzzyPrefixLength"), gs("interface"), gi("port"))
  }
  ...
  def main(args: Array[String]): Unit = {
    val parser = new scopt.OptionParser[CliOption]("gnaf-indexer") {
      ...
      opt[Int]('f', "minFuzzyLength") action { (x, c) =>
        c.copy(fuzzyMinLength = x)
      } text (s"min client may request for min query term length for fuzzy match, default ${defaultCliOption.fuzzyMinLength}")
  ...
  def validationBuf(c: CliOption, qp: QueryParam): ListBuffer[String] = {
    ...
    qp.fuzzy.foreach { f =>
      if (f.minLength < c.fuzzyMinLength) b += s"fuzzy minLength = ${f.minLength} less than min of ${c.fuzzyMinLength}"
